...
exports.DahuaPlatform = void 0;
const settings_1 = require("./settings");
const api_1 = require("./dahua/api");
const discovery_1 = require("./dahua/discovery");
const events_1 = require("./dahua/events");
...
class DahuaPlatform {
    ...
    async initialize() {
        ...
        const protocol = this.platformConfig.secure ? 'https' : 'http';
        const port = this.platformConfig.port || settings_1.DEFAULT_PLATFORM_CONFIG.port;
        this.log.info(`Connecting to Dahua NVR at ${protocol}://${this.platformConfig.host}:${port}`);
        this.api_client = new api_1.DahuaApi(this.platformConfig.host, this.platformConfig.port || settings_1.DEFAULT_PLATFORM_CONFIG.port, this.platformConfig.secure || settings_1.DEFAULT_PLATFORM_CONFIG.secure, this.platformConfig.username, this.platformConfig.password, this.log);
        this.discovery = new discovery_1.DahuaDiscovery(this.api_client, this.platformConfig.host, this.platformConfig.port || settings_1.DEFAULT_PLATFORM_CONFIG.port, this.platformConfig.secure || settings_1.DEFAULT_PLATFORM_CONFIG.secure, this.platformConfig.username, this.platformConfig.password, this.log);
        try {
        ...
        for (const channel of discoveredChannels) {
            ...
            if (existing) {
                ...
                if (channel.deviceInfo) {
                    let updated = false;
                    if (!existing.manufacturer || existing.manufacturer === 'Dahua') {
                        existing.manufacturer = channel.deviceInfo.manufacturer || 'Dahua';
                        updated = true;
                    }
    ...
    createCameraConfig(channelId, name, streamType, deviceInfo) {
        ...
        const manufacturer = deviceInfo?.manufacturer || settings_1.DEFAULT_CAMERA_CONFIG.manufacturer;
        const model = deviceInfo?.model || settings_1.DEFAULT_CAMERA_CONFIG.model;
    ...
    startMotionListener() {
        if (!this.api_client)
            return;
        this.events = new events_1.DahuaEvents(this.api_client, this.log, this.platformConfig.debugMotion || false);
    ...
    shutdown() {
        this.log.info('Shutting down Dahua platform...');
        if (this.events)
            this.events.stop();
...
exports.DahuaPlatform = DahuaPlatform;
