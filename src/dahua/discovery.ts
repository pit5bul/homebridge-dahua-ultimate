...
export class DahuaDiscovery {
  ...
  async getDeviceInfo(): Promise<{
    ...
    try {
      ...
      const sysInfo = await this.api.get<Record<string, string>>('/cgi-bin/magicBox.cgi?action=getSystemInfo');
  ...
  async discoverChannels(): Promise<DiscoveredChannel[]> {
    try {
      ...
      const encodeConfig = await this.api.get<Record<string, string>>('/cgi-bin/configManager.cgi?action=getConfig&name=Encode');
      const titleConfig = await this.api.get<Record<string, string>>('/cgi-bin/configManager.cgi?action=getConfig&name=ChannelTitle');

      // Parse channels from Encode configuration
      // Format: table.Encode[N].MainFormat[0].VideoEnable=true
      const channelMap = new Map<number, { enabled: boolean; name?: string }>();
