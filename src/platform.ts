...
import { DahuaDiscovery } from './dahua/discovery';
import { DahuaEvents, MotionEventCallback } from './dahua/events';
...
export class DahuaPlatform implements DynamicPlatformPlugin {
  ...
  private api_client?: DahuaApi;
  private discovery?: DahuaDiscovery;
  private events?: DahuaEvents;
  private ffmpegPath = '';
  private ffprobePath = '';
  private readonly platformConfig: DahuaPlatformConfig;
  ...
  constructor(
    ...
    this.platformConfig = { ...DEFAULT_PLATFORM_CONFIG, ...config } as DahuaPlatformConfig;
  ...
  private async initialize(): Promise<void> {
    ...
    const port = this.platformConfig.port || DEFAULT_PLATFORM_CONFIG.port;
    this.log.info(`Connecting to Dahua NVR at ${protocol}://${this.platformConfig.host}:${port}`);

    this.api_client = new DahuaApi(
  ...
  private async createCameraAccessory(camera: CameraConfig): Promise<void> {
    ...
    if (this.discovery) {
      ...
      let encoderType: 'software' | 'vaapi' | 'quicksync' | 'nvenc' | 'amf' | 'videotoolbox' | 'v4l2';
      let vcodec: string;
      ...
      if (camera.videoConfig.encoder) {
        ...
        const codecToEncoder: Record<string, typeof encoderType> = {
          'libx264': 'software',
          'h264_nvenc': 'nvenc',
          'h264_qsv': 'quicksync',
          'h264_vaapi': 'vaapi',
  ...
  private startMotionListener(): void {
    ...
    this.events = new DahuaEvents(this.api_client, this.log, this.platformConfig.debugMotion || false);
